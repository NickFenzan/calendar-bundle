...
//        $cols = $columnRepo->findBy(array('legacy_provider'=>13));
        $cols = $columnRepo->findBy(array('legacy_provider'=>10),array('id'=>'DESC'));
        if($cols){
            echo $cols[0]->getId(). ' - '. $cols[0]->getName();
        }
        return new Response();
    }
...
//        print_r($users);
        ...
        /**
     * @Route("/importNewAppointments", name="apptImportNew")
     */
    public function importNewAction() {
        ...
        if ($workingAppts) {
            ...
            $em->flush();
//            return new Response('Success');
            return $this->redirectToRoute('apptImportNew');
        } else {
            return new Response('Success');
        }
//        print_r($users);
